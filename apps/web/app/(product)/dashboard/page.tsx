...
export default async function Dashboard() {
  ...
  return <div className="dashboard-page">
    ...
    {repositories.length > 0 && <section className="repository-maps"><div className="product-kicker">Repository intelligence</div><div>{repositories.flatMap((repository) => { const [owner, repo] = repository.fullName.split("/"); if (!owner || !repo) return []; const base = `/repositories/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}`; return [<a key={`${repository.repositoryId}:map`} href={`${base}/compatibility`}><b>{repository.fullName}</b><span>Open compatibility map →</span></a>, <a key={`${repository.repositoryId}:history`} href={`${base}/recurrence`}><b>{repository.fullName}</b><span>View recurrence history →</span></a>]; })}</div></section>}
    <section className="checks-section" id="recent-checks"><div className="product-section-heading"><div><div className="product-kicker">Repository activity</div><h2>Recent checks</h2></div><span>{list.length} shown</span></div>
      {list.length ? <div className="check-list"><div className="table-head"><span>Pull requests and finding</span><span>Combined result</span><span>Relationship</span><span>When tested</span></div>{list.map((job) => {
        const r = job.result;
        ...
        const relationship = r?.compatibility?.[0];
        const related = r?.comparedPullRequests.map((pr) => `PR #${pr}`).join(", ");
        const relationshipFallback = related ? job.status === "failed" ? "Conflict found in this run" : "Pair passed in this run" : "No related PR tested";
        return <a className="check-row" href={`/jobs/${job.id}`} key={job.id}><span className="repo"><b>{job.owner}/{job.repo}</b><small>PR #{job.prNumber}{related ? ` tested with ${related}` : " tested alone"} · {source}</small><small>{categories.join(" · ") || (job.status === "passed" ? "No blocking migration finding" : "Open for the verified cause")}</small></span><span><span className={`status ${job.status}`}><i/>{job.status === "failed" ? "merge blocked" : job.status === "passed" ? "ready in tested scope" : job.status}</span><small className="explanation-label">{r?.explanationStatus === "pending" ? "Ollama preparing explanation…" : r?.explanation?.source === "ollama" ? "Plain-language review ready" : r?.explanation?.fallbackReason ? "Database evidence; AI unavailable" : "Database evidence ready"}</small></span><span className="table-value relationship-value">{relationship ? <><strong>{relationship.status.replaceAll("_", " ")}</strong><small>{relationship.pullRequests.map((pr) => `#${pr}`).join(" ↔ ")}</small></> : <><strong>{relationshipFallback}</strong>{related && <small>PR #{job.prNumber} ↔ {related.replaceAll("PR ", "")}</small>}</>}</span><time className="table-value">{new Date(job.createdAt).toLocaleString()}</time></a>;
