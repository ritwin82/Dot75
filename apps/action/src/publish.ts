...
import { z } from "zod";
import { DOT75_CHECK_NAME, getTextFile, updateCheck, upsertStickyComment,validationIdempotencyKey } from "@localmesh/github";
...
export async function publishResults(octokit: Octokit, context: PublishContext, artifact: unknown): Promise<{ published: number; skipped: number }> {
  ...
  for (const target of targets) {
    ...
    const externalId = validationIdempotencyKey(owner,repo,target.prNumber,target.headSha,target.baseSha,target.result.provenance?.engineVersion??ENGINE_VERSION);
    const { data: existing } = await octokit.checks.listForRef({ owner, repo, ref: target.headSha, check_name: DOT75_CHECK_NAME, filter: "latest", per_page: 100 });
    const previous = existing.check_runs.find((check) => check.external_id === externalId && check.app?.slug === "github-actions");
    const created=previous??(await octokit.checks.create({ owner, repo, name: DOT75_CHECK_NAME, head_sha: target.headSha, external_id: externalId, status: "queued" })).data;
    const id=created.id;
