...
export async function prepareRun(octokit: Octokit, event: ActionEnvelope["event"], payload: unknown, run: { id: number; attempt: number }, cache?: DiscoveryCache): Promise<PreparedRun> {
  ...
  if (config.migrations.adapter === "raw-sql" && (config.migrations.up_pattern !== "*.up.sql" || config.migrations.down_pattern !== "*.down.sql")) throw new Error("The raw SQL adapter requires *.up.sql and *.down.sql patterns.");
